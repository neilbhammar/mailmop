-- Fix premium upsell email trigger to actually send emails
-- The previous migration removed the HTTP call functionality but didn't replace it
...
'Fixed to actually call the send-premium-upsell-email edge function via HTTP instead of just logging. Uses configurable URL with fallback.'; 